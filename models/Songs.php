...
class Songs extends Model
{
    ...
    public function GetSongsByNameLike($name)
    {
        return Core::getInstance()->getDB()->select("song", '*', ['title' => $name], ['title' => 'ASC'], null, null, 'full');
    }
    ...
    public function ChangePhoto($id, $fileName)
    {
        ...
        if (is_file($folder . $song['image']) && is_file($folder . $fileName) && $fileName != 'default_song.png')
    ...
    public function AddSongs($row)
    {
        ...
        $rowFiltered["image"] = 'default_song.png';
    ...
    public function GetLastSongs($count, $page)
    {
        return Core::getInstance()->getDB()->select('song', '*', null, ['date' => 'DESC'], $count, ($page - 1) * $count);
    }
